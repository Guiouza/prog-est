...
void hanoi(int *v_init, int *v_auxi, int *v_final, unsigned int size, unsigned int n);
...
int main(int argc, char const *argv[])
{
    ...
    vetor_A = malloc(size * sizeof(int)); // aloca memória sem limpar
    vetor_T = calloc(size, sizeof(int));  // aloca memória, mas limpa antes.
    vetor_B = calloc(size, sizeof(int));  // aloca memória, mas limpa antes.
...
void print_tower(unsigned int size)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        printf("%d, ", vetor_A[i]); // imprime os números
    }
...
void hanoi(int *v_init, int *v_auxi, int *v_final, unsigned int size, unsigned int n)
{
    ...
    else
    {
        ...
        hanoi(v_init, v_final, v_auxi, size, n - 1);
        ...
        hanoi(v_auxi, v_init, v_final, size, n - 1);
    }
}